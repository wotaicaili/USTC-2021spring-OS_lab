...
int help_func(int argc, unsigned char **argv) {
    ...
    if (argc > 2) {
        myPrintf(0x7, "Illegal number of parameters for command \"help\"!\n");
        return 0;
    }
...
int cmd_func(int argc, unsigned char **argv) {
    ...
    if (argc > 1) {
        myPrintf(0x7, "Illegal number of parameters for command \"cmd\"!\n");
        return 1;
    }
...
void initShell(void) {
    addNewCmd("exit", exit_func, NULL, "exit : Exit the shell task.");
